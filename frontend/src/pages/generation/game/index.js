...
function Game() {
  ...
  useEffect(() => {
    if (guesses === null && pokemons === null) {
      let allPokemons = POKEDEX.slice(
        GEN_SLICE[generation].start,
        GEN_SLICE[generation].end + 1
      );
      if (difficulty !== 'master') {
        if (DIFFICULTY[difficulty].random) {
          ...
          const slotEnd =
            Math.floor(Math.random() * max) + GEN_SLICE[generation].start;
  ...
  useEffect(() => {
    if (
      gameState === 'progress' &&
      guesses.every((item) => item !== '' && !loading)
    ) {
      setIsActive(true);
    }
  ...
  useEffect(() => {
    if (
      guesses !== null &&
      guesses.length >= 30 &&
      guesses.every((item) => item === '')
    ) {
      setGameState('complete');
      setIsActive(false);
      setTimeout(() => {
        navigate(`/generation/${generation}/${difficulty}/game/results`, {
          state: { score: time },
        });
      }, [2500]);
    }
  ...
  function handleSubmit(e) {
    ...
    if (input !== '') {
      if (guesses.includes(input.toLowerCase())) {
        ...
        pokemons.find(({ name }) => name === input.toLowerCase()) &&
        !guesses.includes(input.toLowerCase())
      ) {
        setReaction('neutral');
        setInput('');
        setTimeout(() => {
  ...
  function handleGiveUp() {
    navigate(`/generation/${generation}/${difficulty}/game/incomplete`, {
      state: {
        time: time,
        guesses: guesses,
        pokemons: pokemons,
    ...
    <motion.div
      ...
      {!loading && (
        <div
          ...
          }  select-none flex space-x-8 mb-8`}>
          <button
            onClick={() => navigate(`/generation/${generation}/difficulty`)}
            className='w-[100px] border-[10px] border-[#707176] text-[#707176] font-arcade font-semibold flex-0   bg-white rounded-xl flex flex-col space-y-1 items-center justify-center transition-all duration-150 ease-in-out active:scale-90'>
            <AiFillPlayCircle className='text-2xl rotate-180' />
            <span>Back</span>
          </button>
          <Stopwatch time={time} setTime={setTime} isActive={isActive} />
          <button
            onClick={() => setGiveUp(true)}
            className='w-[100px] border-[10px] border-[#cf3838] text-[#cf3838] font-arcade font-semibold flex-0   bg-white rounded-xl flex flex-col space-y-1 items-center justify-center transition-all duration-150 ease-in-out active:scale-90'>
